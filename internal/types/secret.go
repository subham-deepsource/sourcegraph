...
func redactionInfo(cfg interface{}) ([]redactableField, error) {
	...
	case *schema.GoModulesConnection:
		return []redactableField{stringArrayField{
			path:  []string{"urls"},
			field: &cfg.Urls,
			redactor: func(path []string, field *string) redactableField {
...
func (f stringArrayField) Redact(input string) (string, error) {
	return f.apply(input, func(field redactableField) (string, error) {
...
func (f stringArrayField) apply(input string, op func(redactableField) (string, error)) (string, error) {
	...
	for i := range array {
		...
		if input, err = op(f.redactor(path, &(*f.field)[i])); err != nil {
			return input, err
		}
	}
...
func (f urlField) Redact(input string) (string, error) {
	...
	return jsonc.Edit(input, redacted.String(), f.path...)
}
...
func (f urlField) Unredact(input string) (string, error) {
	...
	redacted, err := url.Parse(stringValue)
	if err != nil {
		return input, errors.Wrap(err, "failed to parse redacted url")
	}

	// if the field has been edited we should skip unredaction to allow edits
	if password, ok := redacted.User.Password(); ok && password != RedactedSecret {
		// using unicode zero width space might mean the user includes it when editing still, we strip that out here
		return jsonc.Edit(input, strings.ReplaceAll(password, RedactedSecret, ""), f.path...)
	}
